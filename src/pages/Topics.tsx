...
import { mockTopicSources } from '@/data/seedData';
        ...
        {mockTopicSources.slice(0, 3).map((source, idx) => (
          <Card key={source.domain} className="p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <Badge variant={source.type === 'owned' ? 'default' : 'secondary'}>
                {source.type}
              </Badge>
            </div>
            <div className="mb-2">
            ...
            <div className="text-sm text-muted-foreground">of total citations</div>
          </Card>
        ))}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {mockTopicSources.map((source) => (
                <TableRow key={source.domain} className="hover:bg-muted/50 cursor-pointer">
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <a
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {source.topPrompts.slice(0, 2).map((prompt, idx) => (
